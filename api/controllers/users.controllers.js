...
module.exports.register = function(req, res) {
	...
	User.create({
		...
		transporter.sendMail({
		    from: '"Tutorial Shaala" <tutorialshaala@gmail.com>',
		    to: email,
		    subject: 'Tutorial Shaala activation',
		    text: 'Hello ' + name + ',Thank you for Registering at TutorialShaala.com. Please clcik on the link to complete your activation : https://tutorialshaala.herokuapp.com/activate/' + temporarytoken,
		    html: 'Hello<strong> ' + name + '</strong>,<br><br>Thank you for Registering at TutorialShaala.com. Please clcik on the link below to complete your activation :<br><br><a href="https://tutorialshaala.herokuapp.com/activate/' + temporarytoken + '">https://tutorialshaala.herokuapp.com/activate/</a>' 
		},function(err, User){
			if(err){
			...
			user.save(function(err) {
				if (err) {
					...
					transporter.sendMail({
						    ...
						    text: 'Hello ' + user.name + ', You recently requested a new account activation link. Please click on the following link to complete your activation: https://tutorialshaala.herokuapp.com/activate/' + user.temporarytoken,
						    html: 'Hello<strong> ' + user.name + '</strong>,<br><br>You recently requested a new account activation link. Please click on the link below to complete your activation:<br><br><a href="https://tutorialshaala.herokuapp.com/activate/' + user.temporarytoken + '">https://tutorialshaala.herokuapp.com/activate/</a>'
					    },function(err, User){
							if(err){
								res.status(401).json(err);
							} else{
					...
					user.save(function(err) {
						if (err) {
	                       ...
	                       transporter.sendMail({
						        ...
						        text: 'Hello ' + user.name + ', You recently requested a password reset link. Please click on the link below to reset your password:<br><br><a href="https://tutorialshaala.herokuapp.com/reset/' + user.resettoken,
								html: 'Hello<strong> ' + user.name + '</strong>,<br><br>You recently requested a password reset link. Please click on the link below to reset your password:<br><br><a href="https://tutorialshaala.herokuapp.com/reset/' + user.resettoken + '">https://tutorialshaala.herokuapp.com/reset/</a>'
						    },function(err, User){
								if(err){
									res.status(401).json(err);
								} else{
